...
export async function fetchFixturesForLeagues(): Promise<Match[]> {
  try {
    ...
    return data.map((row: { [k: string]: unknown }): Match => ({
      ...
      leagueIcon: (row.league_logo as string | undefined) ? String(row.league_logo as string) : undefined,
      ...
      streamUrl: (row.live_url as string | undefined) ? String(row.live_url as string) : "",
